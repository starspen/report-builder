...
import {
  ...
  deleteCSMasterItemSetup,
  deleteCSMasterSetup,
} from "@/action/customer-service-master";
import EditItemMasterForm from "../edit-item-master-form";
import NewItemMasterForm from "../new-item-master-form";

interface CSItemMasterDataTableToolbarProps {
  table: Table<any>;
  user: any;
}

export function CSItemMasterDataTableToolbar({ table, user }: CSItemMasterDataTableToolbarProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteCSMasterItemSetup(id),
    onSuccess: (data) => {
    ...
    <div className="flex flex-1 flex-wrap items-center gap-2">
      ...
      {selectedRows.length === 1 && (
        <>
          <EditItemMasterForm
            itemData={{
              ic_flag: selectedRows[0].ic_flag,
              item_cd: selectedRows[0].item_cd,
              ...
              charge_amt: selectedRows[0].charge_amt,
          ...
          >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  Hapus item master dengan kode:{" "}
                  {selectedRows[0].item_cd}?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  Tindakan ini tidak dapat dibatalkan. Data item master akan
