...
import { updateCSMasterFeedbackSetup } from "@/action/customer-service-master";
...
const feedbackSetupFormSchema = z.object({
  code: z
    ...
    .max(20, { message: "Code maksimal 20 karakter" }),
  descs: z
    .string()
...
interface EditFeedbackSetupFormProps {
  feedbackData: {
    code: string;
    descs: string;
    rowID: string;
...
export default function EditFeedbackSetupForm({ feedbackData, user }: EditFeedbackSetupFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      descs: feedbackData.descs,
    },
  });

  // Mutation untuk update feedback
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => updateCSMasterFeedbackSetup(id, data),
    onSuccess: (data) => {
    ...
    onError: (error: Error) => {
      toast.error(error.message || "Gagal mengupdate feedback");
    },
  });
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            ...
            <DialogDescription>
              Update information for the selected feedback
            </DialogDescription>
          </DialogHeader>
