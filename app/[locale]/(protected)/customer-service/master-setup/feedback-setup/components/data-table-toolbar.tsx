...
import {
  deleteCSMasterFeedbackSetup,
} from "@/action/customer-service-master";
import EditFeedbackSetupForm from "../edit-feedback-setup-form";
import NewFeedbackSetupForm from "../new-feedback-setup-form";
...
export function DataTableToolbar({ table, user }: DataTableToolbarProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteCSMasterFeedbackSetup(id),
    onSuccess: (data) => {
  ...
  return (
    <div className="flex flex-1 flex-wrap items-center gap-2">
      <Input
        placeholder="Cari feedback setup..."
      ...
      <NewFeedbackSetupForm user={user} />

      {/* Form Dialog Edit Section */}
      {selectedRows.length === 1 && (
        <>
          <EditFeedbackSetupForm
            feedbackData={{
          ...
          >
            <AlertDialogTrigger asChild>
              ...
              >
                ...
                Hapus Feedback Setup
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  Hapus feedback setup dengan kode:{" "}
                  {selectedRows[0].code}?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  Tindakan ini tidak dapat dibatalkan. Data feedback setup akan
