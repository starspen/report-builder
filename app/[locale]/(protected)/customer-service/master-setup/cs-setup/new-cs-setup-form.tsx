...
const csSetupFormSchema = z.object({
  ...
  descs: z.string().min(1, { message: "Service description harus diisi" }).max(255, { message: "Service description maksimal 100 karakter" }),
...
interface NewCSSetupFormProps {
  user?: any;
}

export default function NewCSSetupForm({ user }: NewCSSetupFormProps) {
  ...
  // Query untuk Transaction Type data dengan optimasi
  ...
  // Mutation untuk create CS Setup
  const createMutation = useMutation({
    mutationFn: createCSMasterSetup,
    onSuccess: (data) => {
    ...
    onError: (error: Error) => {
      toast.error(error.message || "Gagal menambah service");
    },
  });
  ...
  const onSubmit = async (data: CSSetupFormValues) => {
    try {
      ...
      const validationErrors = validateSetupData(setupData);
      ...
      toast.error("Gagal menambah service");
    }
  };

  // Options untuk Section dropdown
  ...
  // Options untuk Category dropdown
  ...
  // Options untuk Transaction Type dropdown
  ...
  // Options untuk Currency dropdown
  ...
  // Options untuk Tax Code dropdown
      ...
      <DialogContent className="sm:max-w-[600px]" size="md">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter information for the new service
            </DialogDescription>
          </DialogHeader>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <SelectWithSearch
                      ...
                      disabled={isLoadingSection || createMutation.isPending}
                    />
                  )}
                />
                {errors.section_cd && (
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <SelectWithSearch
                      ...
                      disabled={isLoadingCategory || createMutation.isPending}
                    />
                  )}
                />
                {errors.category_cd && (
