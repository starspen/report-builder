...
export function DataTableToolbar({ table, user }: DataTableToolbarProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteCSMasterLocation(id),
    onSuccess: (data) => {
    ...
    <div className="flex flex-1 flex-wrap items-center gap-2">
      ...
      {selectedRows.length === 1 && (
        <>
          <EditLocationForm
            locationData={{
              location_cd: selectedRows[0].location_cd,
          ...
          >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  Hapus location dengan kode:{" "}
                  {selectedRows[0].location_cd}?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  Tindakan ini tidak dapat dibatalkan. Data location akan
