...
import { createCSMasterSvspec, getCSMasterComplainSource } from "@/action/customer-service-master";
...
export default function NewSvspecForm({ user }: NewSvspecFormProps) {
  ...
  // Helper function untuk reset form
  const resetForm = useCallback(() => {
    reset(DEFAULT_FORM_VALUES);
  }, [reset]);

  // Mutation untuk create svspec
  const createMutation = useMutation({
    mutationFn: createCSMasterSvspec,
    onSuccess: (data) => {
      toast.success("SV SPEC berhasil ditambahkan");
    ...
    onError: (error: Error) => {
      toast.error(error.message || "Gagal menambah SV SPEC");
    },
  });
  ...
  const onSubmit = async (data: SvspecFormValues) => {
    try {
      const svspecData = prepareFormData(data);
      createMutation.mutate(svspecData);
      console.log({svspecData});
    } catch (error) {
      console.error("Error submitting form:", error);
      toast.error("Gagal menambah SV SPEC");
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto" size="md">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Add New SV SPEC</DialogTitle>
            <DialogDescription>
              Enter information for the new SV SPEC configuration
            </DialogDescription>
          </DialogHeader>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <SelectWithSearch
                      ...
                      disabled={!watch("entity_cd") || isLoadingDocTypeData || createMutation.isPending}
                    />
                  )}
                />
                {errors.prefix && (
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <SelectWithSearch
                      ...
                      disabled={!watch("entity_cd") || isLoadingTenantDocTypeData || createMutation.isPending}
                    />
                  )}
                />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <SelectWithSearch
                        ...
                        disabled={isLoadingTrxTypeICData || createMutation.isPending || watch("link") === "0"}
                      />
                    )}
                  />
                  {errors.trx_type && (
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <SelectWithSearch
                      ...
                      disabled={isLoadingComplainSourceData || createMutation.isPending}
                    />
                  )}
                />
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={createMutation.isPending}>
              {createMutation.isPending ? "Adding..." : "Add SV SPEC"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
