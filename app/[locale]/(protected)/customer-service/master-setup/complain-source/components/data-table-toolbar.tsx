...
import {
  deleteCSMasterComplainSource,
} from "@/action/customer-service-master";
import EditComplainSourceForm from "../edit-complain-source-form";
import NewComplainSourceForm from "../new-complain-source-form";
...
export function DataTableToolbar({ table, user }: DataTableToolbarProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteCSMasterComplainSource(id),
    onSuccess: (data) => {
  ...
  return (
    <div className="flex flex-1 flex-wrap items-center gap-2">
      <Input
        placeholder="Cari complain source..."
      ...
      <NewComplainSourceForm user={user} />

      {/* Form Dialog Edit Section */}
      {selectedRows.length === 1 && (
        <>
          <EditComplainSourceForm
            complainSourceData={{
          ...
          >
            <AlertDialogTrigger asChild>
              ...
              >
                ...
                Hapus Complain Source
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  Hapus complain source dengan kode:{" "}
                  {selectedRows[0].complain_source}?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  Tindakan ini tidak dapat dibatalkan. Data complain source akan
