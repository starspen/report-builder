...
import { complainSourceHeaders, ComplainSourceDataProps } from "../data";
import { DataTableColumnHeader } from "./data-table-column-header";
import { Badge } from "@/components/ui/badge";
import { format } from "date-fns";
import { id } from "date-fns/locale";

export const complainSourceColumns: ColumnDef<ComplainSourceDataProps>[] = [
  ...complainSourceHeaders.map((header) => ({
    accessorKey: header.accessorKey,
    header: ({ column }: { column: Column<ComplainSourceDataProps> }) => (
      <DataTableColumnHeader column={column} title={header.header} />
    ),
    cell: ({ row }: { row: Row<ComplainSourceDataProps> }) => {
      const value = row.getValue(header.accessorKey);

      if (header.accessorKey === "complain_source") {
        return (
