...
export default function ModulesPage() {
  ...
  useEffect(() => {
    ...
    const updatedModule = modules.find((m: { id: any; }) => m.id === selectedModule.id)
    if (!updatedModule) return

    setSelectedModule(updatedModule)

    if (selectedRole) {
      const updatedRole = updatedModule.roles.find((r: { id: any; }) => r.id === selectedRole.id)
          ...
          <div className="grid grid-cols-2 gap-6">
                  ...
                  {selectedModule.roles.map((r: any) => (
                    ...
                    >
                      ...
                      <div className="flex space-x-2">
                        <EditRoleDialog role={r} moduleId={selectedModule.id} />
                        <DeleteRoleDialog role={r} moduleId={selectedModule.id} />
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <section>
              <div className="flex justify-between items-center mb-2">
                ...
                {selectedModule && <AddMenuDialog moduleId={selectedModule.id} />}
              </div>
                ...
                <ul className="space-y-1">
                  {selectedModule.menus.map((m: any) => (
                    ...
                    >
                      <span>{m.name}</span>
                      <div className="flex space-x-2">
                        <EditMenuDialog menu={m} moduleId={selectedModule.id} />
                        <DeleteMenuDialog menu={m} moduleId={selectedModule.id}/>
                      </div>
                    </li>
                  ))}
                </ul>
