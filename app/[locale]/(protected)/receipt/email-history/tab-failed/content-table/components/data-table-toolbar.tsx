...
import { resendReceiptEmail } from "@/action/receipt-action";
...
export function DataTableToolbar({
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      const result = await resendReceiptEmail(docNo, processId, email);
      return result;
    },
    onMutate: () => {
      setIsLoading(true);
    ...
    onSuccess: (result) => {
      if (result.statusCode === 200 || result.statusCode === 201) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["receipt-email-history-failed"],
        });
      } else {
  ...
  const handleResendingReceiptEmail = async () => {
      ...
      <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <Button
              className="relative"
              onClick={handleResendingReceiptEmail}
              disabled={isLoading}
            >
              {isLoading ? (
                "Resending..."
