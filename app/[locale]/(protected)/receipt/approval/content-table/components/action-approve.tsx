...
import { submitReceiptApproval } from "@/action/receipt-action";
...
export function ActionApprove({
  ...
  const handleApprove = async () => {
    if (selectedRows.size === 0) {
      toast.error("please select a receipt to be approved");
      return;
    }
    ...
    if (successCount > 0) {
      toast.success(`Successfully approved ${successCount} receipt(s).`);
    }
    if (failCount > 0) {
      toast.error(`Failed to approve ${failCount} receipt(s).`);
      console.error("Detail errors:", failMessages);
    }

    queryClient.invalidateQueries({ queryKey: ["receipt-approval-by-user"] });
  ...
  return (
    <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Are you sure you want to approve the selected receipts?
          </AlertDialogTitle>
          <AlertDialogDescription>
