...
import { regenerateInvoiceEmail } from "@/action/invoice-action";
...
export function DataTableToolbar({
  ...
  // console.log(table.getRow(String(rowId))?.original)
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      const result = await regenerateInvoiceEmail(docNo, processId);
      return result;
    },
    onMutate: () => {
      setIsLoading(true);
  ...
  const handleRegenerateInvoiceEmail = async () => {
    ...
    <div className="flex flex-1 flex-wrap items-center gap-2">
      ...
      <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Are you sure you want to regenerate the selected data?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Please confirm if you want to proceed. This action will regenerate
              the selected data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <Button
              className="relative"
              onClick={handleRegenerateInvoiceEmail}
              disabled={isLoading}
            >
              {isLoading ? (
                "Regenerating..."
